...
**连线的 `from` / `to` 必须指向复合卡片的最外层 frame 的 id，严禁指向卡片内部的子元素（icon、text、badge 等）。**

复合卡片是一个 frame 包含 icon + text + badge 等子元素。连线应连接到整张卡片，而非卡片内部的某个零件。

```json
// ❌ 错误：连线指向了卡片内部的 icon 或 text
{ "type": "frame", "id": "card_api", "children": [
  { "type": "icon", "id": "icon_api", "name": "server" },
  { "type": "text", "id": "text_api", "text": "API Gateway" }
]}
{ "type": "connector", "connector": { "from": "icon_api", "to": "icon_db" } }

// ✅ 正确：连线指向卡片最外层 frame 的 id
{ "type": "frame", "id": "card_api", "children": [
  { "type": "icon", "name": "server" },
  { "type": "text", "text": "API Gateway" }
]}
{ "type": "connector", "connector": { "from": "card_api", "to": "card_db" } }
```

**实操规则**：
- 只给复合卡片的**最外层 frame** 设置 `id`，卡片内部子元素（icon、text、badge frame）**不要设置 id**
- 这样从根源上避免连线误连到内部元素
- Dagre `edges` 中的节点 ID 同理，必须是卡片外层 frame 的 id
