...
- 所有节点有边框（borderWidth 按层级 1-2）
- 间距不粘连（gap >= 8，有连线时 >= 40，详见 [connectors.md](connectors.md)）
- 文字在背景上清晰可读（正文 fontSize >= 14，复合卡片内的副标题/元信息允许 10-12，但必须满足下方对比度要求）
- 连线用灰色（#BBBFC4），不抢节点注意力

### 无障碍（Accessibility）

**对比度**：文字与其直接背景色之间的对比度必须满足 WCAG 2.1 AA 标准：

| 文字类型 | 最低对比度 | 说明 |
|---------|----------|------|
| 正文（>= 14px） | 4.5:1 | 常规大小文字 |
| 大标题（>= 18px bold 或 >= 24px） | 3:1 | 较大文字要求较低 |
| 副标题/元信息（10-12px） | 4.5:1 | 字小更需要高对比 |

**已知 bad case（禁止出现）**：

| 组合 | 对比度 | 问题 |
|------|--------|------|
| 浅黄底 `#FEF1CE` + 白字 `#FFFFFF` | ~1.1:1 | 完全看不清 |
| 浅蓝底 `#F0F4FC` + 浅灰字 `#8F959E` | ~2.5:1 | 模糊难读 |
| 浅绿底 `#DFF5E5` + 绿字 `#509863` | ~3.2:1 | 正文不达标 |

> 安全做法：浅色分组背景上的文字统一用 `#1F2329`（深色）或 `#646A73`（辅助），不要用分组主色做正文 textColor。分组主色只用于**分区标签**（字号 >= 16 且 bold，属于大标题，3:1 即可）。

**色觉障碍**：

- 不要仅靠颜色区分信息——同时使用**边框样式（solid/dashed）、形状（rect/胶囊 frame/diamond）、图标、文字标签**辅助区分
- 经典色板的蓝/紫/绿/黄/红五色对红绿色盲友好度不一。蓝色和黄色在各类色觉障碍中辨识度最高，红色和绿色容易混淆。当图表有红绿并存的分组时，务必用不同的边框样式或形状来辅助区分
- 连线如需区分多条路径，优先用 `lineStyle`（solid/dashed/dotted）而非颜色

### 边框粗细层级（Border Weight Hierarchy）

边框粗细应随嵌套层级递减，形成"外重内轻"的视觉纵深：

| 层级 | borderWidth | borderRadius | 用途 |
|------|-------------|-------------|------|
| 层容器（最外） | 2 | 12-16 | 分区背景框 |
| 内容卡片 | 1 | 8-10 | 复合卡片节点 |
| 强调/表头节点 | 2 | 8 | 需要突出的节点 |
| 状态标签（Badge） | 1 | 4 | 卡片内部的小标签 |

> 不要所有节点都用 `borderWidth: 2`。层容器用 2，内容卡片用 1，标签用 1，这样才能看出"谁包含谁"。

### 边框虚实语义（Border Dash Semantics）

| 样式 | 含义 | 适用场景 |
|------|------|---------|
| `solid` | 确定的、已有的实体 | 默认，所有实体节点 |
| `dashed` | 逻辑分组、区域边界 | K8s 集群、网络分区、功能域、透明子图 |
| `dotted` | 计划中的、可选的、弱关联 | 未来规划、可选组件 |

### 连线粗细层级（Connector Weight Hierarchy）

| 类型 | lineWidth | lineStyle | 用途 |
|------|-----------|-----------|------|
| 主流程/关键路径 | 2 | solid | 核心数据流、主调用链 |
| 次要关系 | 1 | solid | 辅助连接 |
| 异步/事件驱动 | 1 | dashed | 消息推送、回调、Webhook |
| 弱关联/注解 | 1 | dotted | 可选路径、注释箭头 |

### 通用参数

| 参数 | 值 | 为什么 |
|------|---|--------|
| borderWidth | 1-2（见上方层级表） | 外重内轻，形成纵深 |
| borderRadius | 4-16（见上方层级表） | 按嵌套层级递变 |
| gap（最小值） | 8 | 元素不粘连 |
| padding（最小值） | 8 | 内容不贴边 |
| gap（有连线时） | 40 | 给箭头留空间（详见 [connectors.md](connectors.md)） |
| fontSize（正文） | >= 14 | 可读 |
| fontSize（标题） | >= 24 | 醒目 |
| fontSize（辅助） | >= 10 | 复合卡片内部的副标题/技术栈 |
...
内容节点使用 frame 复合卡片（详见 `schema.md`「复合卡片」）。卡片内部结构自由组合，配色遵循以下规则：

- 卡片 `borderWidth: 1`（比外层容器的 2 更轻），`borderRadius: 8-10`
- 卡片 `fillColor: "#FFFFFF"`，边框用分组色的**柔和变体**（见下方表格）
- icon 的 `color` 跟随所属分组的 borderColor
- 有色头部区域用分组的浅色 fillColor（如蓝组用 `#F0F4FC`）
- 卡片内子容器（子项行、badge 等）用中性浅灰底（`#F8F9FA`）区分层次

### textColor 层级

文字颜色应有明确的主次梯度，不要所有文字用同一个颜色：

| 角色 | textColor | bold | 用途 |
|------|-----------|------|------|
| 主标题 | `#1F2329` | **是** | 图表标题、卡片标题 |
| 正文 | `#1F2329` | 否 | 节点内主要文字 |
| 辅助说明 | `#646A73` | 否 | 副标题、技术栈、描述 |
| 元信息 | `#8F959E` | 否 | 版本号、时间戳、英文副标题 |
| 分区标签 | 分组 borderColor | **是** | 用主题色（如蓝色层用 `#5178C6`） |
| 状态标签 | 分组色深色变体 | 否 | Badge 文字（如 `#B8972E`） |

> **关键原则**：标题用 bold + `#1F2329`，副标题用 normal + `#8F959E`，至少形成 2 级文字层次。分区标签用所属分组的主题色作为 textColor，不要全用黑色。

### 卡片内边框的柔和处理

复合卡片的边框不要直接使用分组主色（过于强烈），改用**主色的浅色变体**，让卡片柔和融入分组背景：

| 分组 | 层容器 borderColor | 卡片 borderColor（柔和） | Badge 底色 |
|------|-------------------|----------------------|-----------|
| 蓝色 | `#5178C6` | `#C2D3EE` | `#F0F4FC` |
| 绿色 | `#509863` | `#C8E6CF` | `#F0FDF4` |
| 黄色 | `#D4B45B` | `#E8D9A0` | `#FEF9E7` |
| 紫色 | `#8569CB` | `#CFC4E6` | `#F3F0FA` |
| 红色 | `#D25D5A` | `#E8B4B3` | `#FEF2F2` |

> 层容器用主色边框（`borderWidth: 2`），卡片用浅色边框（`borderWidth: 1`），形成"外强内柔"的视觉纵深。
