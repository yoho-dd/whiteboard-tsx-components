...
5. **含文字节点高度用 fit-content**（详见 [schema.md WBSizeValue](schema.md)）。
...
```bash
DEBUG=1 node ./packages/whiteboard-cli/bin/cli.js -i skeleton.json -o step1.png -l coords.json
```
...
| 分区之间间距     | 24-32px     | —            |
| 同分区内节点间距 | 12-16px     | —            |
| 有连线的节点间距 | >= 40px     | 给箭头留空间（详见 [connectors.md](connectors.md)） |
| 分区内边距       | 16-24px     | —            |
| 侧标签宽度       | 120-180px   | —            |
